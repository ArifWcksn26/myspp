<x-filament-panels::page>
    <x-filament::section>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Gambar & Scan card (image and scancard) on the right side -->
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-gray-400 font-medium mb-1">Scan Card:</label>
                    <img src="{{ $user->scancard ? asset('storage/' . $user->scancard) : asset('images/default-card.png') }}"
                        alt="Scan Card" class="w-32 h-auto object-cover rounded-md border cursor-pointer"
                        x-on:click="$dispatch('open-modal', { id: 'image-modal', image: '{{ $user->scancard ? asset('storage/' . $user->scancard) : asset('images/default-card.png') }}' })">
                </div>
            </div>
        </div>
    </x-filament::section>
